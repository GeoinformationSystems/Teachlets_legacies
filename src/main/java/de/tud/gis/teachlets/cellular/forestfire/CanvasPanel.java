...
//	}//calcPaintRec(MouseEvent e)
	 ...
	 */
	 public void updateFH(int matrixwidth){
	    	matrix = new Cell[Math.abs(this.getHeight()/matrixwidth)][Math.abs(this.getWidth()/matrixwidth)];
...
//	 }
	 ...
	 private void createJPanel(){
			...
			matrix = new Cell[Math.abs(this.getHeight()/gui.getMatrixwidth())][Math.abs(this.getWidth()/gui.getMatrixwidth())];
	 ...
	 public void paintComponent(Graphics g){
		 ...
		 for (int i = 0; i < matrix.length; i++) {
				for (int j = 0; j < matrix[0].length; j++) {
					...
					g.fillRect(j * gui.getMatrixwidth(), i * gui.getMatrixwidth(), gui.getMatrixwidth(), gui.getMatrixwidth());
				}// for
			}// for
