...
public class NetworkAnalysisMain extends JApplet 
{
	...
	private JButton btnStartLocat;
	private JButton btnAlgoLocat;
	private JTextArea taInstrucLocat;
	...
	{
		taInstrucLocat = new JTextArea();
		btnAlgoLocat = new JButton(); 
		...
		taInstrucLocat.setEditable(false);
	...
	private JTextPane createTextPane() 
	{
        ...
        //add elements to styled document according to algorithm
      	try {
			doc.insertString(doc.getLength(), "Standortplanung:", doc.getStyle("heading"));
			doc.insertString(doc.getLength(), _dataModel.getAlgorithm().getDescription(), doc.getStyle("regular"));
			doc.insertString(doc.getLength(), " ", doc.getStyle("buttonStartLocat"));
			doc.insertString(doc.getLength(), "\n\n", doc.getStyle("regular"));
			doc.insertString(doc.getLength(), " ", doc.getStyle("lblInstrucLocat"));
			doc.insertString(doc.getLength(), "\n\n", doc.getStyle("regular"));
			doc.insertString(doc.getLength(), " ", doc.getStyle("buttonAlgoLocat"));
	...
	protected void addStylesToDocument(StyledDocument doc) 
	{
        ...
        s = doc.addStyle("buttonStartLocat", regular);
        ...
        btnStartLocat.addActionListener( new ActionListener() {
			...
			{				
				_dataModel.setActualPhase(DataModel.AlgorPhase.SETENDNODE);
				_dataModel.clearResultLists();
				_dataModel.clearNodeListStartEnd();
				_dataModel.getNodeListAlgo().clear();
				_viewer.setActualInstructionTextArea(taInstrucLocat);
				_viewer.setActualContinueButton(btnAlgoLocat);
        ...
        StyleConstants.setComponent(s, btnStartLocat);
        
        s = doc.addStyle("buttonAlgoLocat", regular);
        StyleConstants.setAlignment(s, StyleConstants.ALIGN_CENTER);
        ...
        btnAlgoLocat.addActionListener( new ActionListener() {
			...
			{				
				...
				Vector<Node> result = _dataModel.getAlgorithm().calculateAlgo(_dataModel);
				_dataModel.setNodeResultList(result);
				_viewer.repaint();
		  } 
		} );
        StyleConstants.setComponent(s, btnAlgoLocat);
        
        //add Label	        
        s = doc.addStyle("lblInstrucLocat", regular);
        StyleConstants.setAlignment(s, StyleConstants.ALIGN_CENTER);
        
        taInstrucLocat.setFont(new Font("SansSerif", Font.BOLD, 12));
        
        StyleConstants.setComponent(s, taInstrucLocat);	    
    }
